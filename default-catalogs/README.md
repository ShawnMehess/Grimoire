...
- `feats-2024.json` — generated by `scripts/compile-2024-content.mjs`
  from `data/feats-2024.json` (the 17 feats in the free 2024 SRD), same
  browsable-reference treatment and same reasoning as `feats.json`
  above. Prerequisite text is assembled from the SRD's structured
  prerequisite data (minimum level / required feature); Effect is the
  SRD's full rules text. Re-run the script any time `data/feats-2024.json`
  is refreshed.
